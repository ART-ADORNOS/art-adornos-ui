...
const ProductList = () => {
    ...
    return (<div className="bg-zinc-100 dark:bg-gray-900 flex-auto text-gray-900 dark:text-white flex flex-col">
        ...
        {loading || loadingTwo ? (<div className="flex items-center justify-center h-96 w-full">
            <Loader/>
        </div>) : (<>
            ...
            {usertype === USER_TYPE.SELLER && (<div className="w-full px-8 py-4 ">
                <CategorySidebar categories={categories}/>
            </div>)}

            {products.length > 0 ? (<div className="container mx-auto px-4 sm:px-8 lg:px-16 py-10">
                <div className="flex flex-wrap gap-4">
                    {products.map((product) => (<ProductCard key={product.id} product={product} usertype={usertype}/>))}
                </div>
            </div>) : (<div

...
const CategoryListModal = ({ isOpen, onClose, categories }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
            <div className="bg-white dark:bg-gray-900 shadow-xl rounded-lg p-6 max-w-md w-full border border-gray-300 dark:border-gray-700 overflow-y-auto">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4 text-center">
                    Categorías
                </h2>
                <ul className="divide-y divide-gray-300 dark:divide-gray-700 pt-4">
                    {categories.map((category) => (
                        <li key={category.id} className="py-3 text-gray-700 dark:text-gray-300 flex justify-between items-center">
                            <span>{category.name}</span>
                            <div className="flex gap-4">
                                <Link to="/register-category" state={{ categoryId: category.id, categoryName: category.name, categoryDescription: category.description }}>
                                    <button
                ...
                <button onClick={onClose} className="mt-6 w-full py-3 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-all focus:outline-none focus:ring">
                    Cerrar
                </button>
            </div>
            {selectedCategory && (

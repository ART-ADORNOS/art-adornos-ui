        ...
        <div className="w-full max-w-full overflow-x-auto no-scrollbar">
            <div className="flex items-center gap-4 p-4 bg-zinc-100 rounded-lg dark:bg-gray-900">
                {industry.map(({value, label}) => {
                    const Icon = INDUSTRY_ICONS[value] || INDUSTRY_ICONS.GENERAL;
                    const color = INDUSTRY_COLORS[value] || INDUSTRY_COLORS.GENERAL;
                    const isActive = activeFilters?.includes(value) || false;
                    return (
                        <button
                            key={value}

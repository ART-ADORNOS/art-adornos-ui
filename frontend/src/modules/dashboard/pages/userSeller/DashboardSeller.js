...
const DashboardSeller = () => {
    ...
    const filteredStartups = activeFilters.length > 0 ? startupData.filter(startup => startup.industry.some(ind => activeFilters.includes(ind))) : startupData;
    ...
    return (<div className="bg-zinc-100 dark:bg-gray-900 flex-auto text-gray-900 dark:text-white flex flex-col">
        ...
        {loading ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-8 mt-10">
            <Loader/>
            <Loader/>
            <Loader/>
        </div>) : (filteredStartups.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-8 mt-10">
                {filteredStartups.map((startup, index) => (<div key={index}>
